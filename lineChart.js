...
function drawChart(querydata, seq) {
  ...
  if(seq == 1)   
    for (var i in querydata) {
        data.lineChart.push({date: querydata[i].time.substring(0, 10), value: querydata[i].temperature, label: querydata[i].time.substring(0, 10) + " [ "+querydata[i].unit+" ]" });
    }
  else if (seq == 2)
    for (var i in querydata) {
         data.lineChart.push({date: querydata[i].time.substring(0, 10), value: querydata[i].weight, label: querydata[i].time.substring(0, 10) + " [ "+querydata[i].unit+" ]" });
    }

  //drawPieChart(     'pieChart',     data.pieChart );
  drawLineChart(    'lineChart'+seq,    data.lineChart );
}